...
import uz.katm.registry.service.InsuranceRegistryService;

import java.util.List;

/** Реестр страховых организаций (перенос InsuranceController монолита). */
@Tag(name = "Registry · Insurance", description = "Реестр страховых организаций")
@RestController
@RequestMapping("/api/v1/registry/insurances")
@RequiredArgsConstructor
@Validated
@PreAuthorize("isAuthenticated()")
public class InsuranceController {

    private final InsuranceRegistryService service;

    @Operation(summary = "Список страховых организаций")
    @GetMapping
    public ResponseEntity<ApiResponse<List<OrgDto>>> list() {
        return ResponseEntity.ok(ApiResponse.ok(service.list()));
    }

    @Operation(summary = "Регистрация страховой организации")
    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<RegistrationResult>> register(@Valid @RequestBody RegisterInsuranceRequest req) {
        return ResponseEntity.ok(ApiResponse.ok(service.register(req)));
    }

    @Operation(summary = "Смена статуса страховой организации")
    ...
    @Operation(summary = "Сброс пароля страховой организации")
